public class LZWBinFa implements Comparable<LZWBinFa> {
	...
	@Override public int compareTo(LZWBinFa masik) 
	{
		try 
    {
			if(this.getMelyseg() < masik.getMelyseg()) 
			{
				return -1;
			} 
      else if (this.getMelyseg() > masik.getMelyseg()) 
      {
				return 1;
			} 
      else if (this.getAtlag() < masik.getAtlag()) 
      {
				return -1;
			} 
      else if (this.getAtlag() > masik.getAtlag()) 
      {
				...
				throw new NotComparable("Error: They are not Comparable!");
			}
		} 
  ...
  public class NotComparable extends Exception //NotComparable OSZTÁLY!
  {
